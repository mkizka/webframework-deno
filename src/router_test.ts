...
Deno.test("Router.(add|match): 文字列でパスパラメータを指定してマッチ", async () => {
...
Deno.test("Router.(add|match): 正規表現でパスパラメータを指定してマッチ", async () => {
  const router = new Router();
  router.add({
    path: /\/hoge\/(?<fuga>.+)/,
    method: "GET",
...
Deno.test("Router.(add|match): 静的なパスにマッチ", async () => {
  const router = new Router();
  router.add({
    path: "/hoge/fuga",
    method: "GET",
...
Deno.test("Router.(add|match): クエリパラメータを含むパスにマッチ", async () => {
  const router = new Router();
  router.add({
    path: "/hoge/fuga",
    method: "GET",
...
Deno.test("Router.(add|match): マッチしない場合は404が返ってくる", async () => {
  const router = new Router();
  router.add({
    path: /\/hoge\/(?<fuga>.+)/,
    method: "GET",
